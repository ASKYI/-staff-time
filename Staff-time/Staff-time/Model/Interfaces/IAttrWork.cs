...
namespace Staff_time.Model.Interfaces
{
    public interface IAttrWork           // todo честно говоря последнее слово неочевидно, то ли это интерфейс для работ(work) над Attr, то ли это это цельный объект AttrWork . Является ли это репозиторием?
    {
        ...
        void Create_AttrValuesFields_ForWork(int WorkID, WorkTypeEnum type);   // todo именование функций, знаки подчёркивания лишние, они ничего не добавляют . Можно в качестве руководства по именованию взять это http://rsdn.org/article/mag/200401/codestyle.XML
